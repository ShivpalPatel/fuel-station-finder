                    ...
                    @foreach ($carModels as $car)
                        <tr class="border-b hover:bg-gray-100">
                            ...
                            <td class="px-4 py-2 text-center">
                                <a href="{{ route('admin.cars.edit', $car->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-lg shadow-md text-sm mr-2">
                                    Edit
                                </a>
                                <form action="{{ route('admin.cars.destroy', $car->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded-lg shadow-md text-sm ml-1">
                                        Delete
                                    </button>
                                </form>
                            </td>
